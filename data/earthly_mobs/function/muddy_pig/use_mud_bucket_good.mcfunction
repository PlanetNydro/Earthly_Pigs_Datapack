...
execute as @e[predicate=earthly_mobs:pig/likes_mud_bucket,predicate=earthly_mobs:pig/wearing_mud_bucket] run data merge entity @s {variant:"earthly_mobs:muddy"}
# removes the bucket and gives it back to the player. also fixes any non-muddy pigs still wearing mud buckets
execute as @e[predicate=earthly_mobs:pig/wearing_mud_bucket] run data merge entity @s {equipment: {saddle:{count: 0, id:"minecraft:air"}}}
give @s bucket
